...
model = fasttext.load_model('/workspace/model/model_query_epoch25_lr0_5_ngram2.bin')
...
def get_categories_query(user_query, threshold=0.5, candidate=5):
    predicted_output = model.predict(user_query, threshold=threshold, k=candidate)
    product_categories = [label.replace('__label__','') for label in predicted_output[0]]
    return product_categories

def search(client, user_query, index="bbuy_products", sort="_score", sortDir="desc"):
    #### W3: classify the query
    product_categories = get_categories_query(user_query)
    print ("Predicted product categories: ",product_categories)
    #### W3: create filters and boosts
    query_filters = []
    if product_categories:
        query_filters.append({
            "terms": {
                "categoryPathIds.keyword": product_categories
            }
        })

    # Note: you may also want to modify the `create_query` method above
    query_obj = create_query(user_query, click_prior_query=None, filters=query_filters, sort=sort, sortDir=sortDir, source=["name", "shortDescription"])
