...
def create_query(user_query, click_prior_query, filters, sort="_score", sortDir="desc", size=10, source=None, querySynonymField = False):
    name_field = "name"
    if querySynonymField:
        name_field = "name.synonym"
    query_obj = {
        'size': size,
        "sort": [
            {sort: {"order": sortDir}}
...
def search(client, user_query, querySynonymField, index="bbuy_products", sort="_score", sortDir="desc"):
    #### W3: classify the query
    #### W3: create filters and boosts
    # Note: you may also want to modify the `create_query` method above
    query_obj = create_query(user_query, click_prior_query=None, filters=None, sort=sort, sortDir=sortDir, source=["name", "shortDescription"], querySynonymField =  querySynonymField)
...
if __name__ == "__main__":
    ...
    general.add_argument('--synonyms', action=argparse.BooleanOptionalAction,
                         help='Query name.synonyms field instead of name field')
    ...
    querySynonymField = False
    if args.synonyms:
        querySynonymField = True
    ...
    for line in fileinput.input('-'):
        ...
        if query == "Exit":
            break
        search(client=opensearch, user_query=query, querySynonymField = querySynonymField, index=index_name)
